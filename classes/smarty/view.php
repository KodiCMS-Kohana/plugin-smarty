...
class Smarty_View extends Kohana_View {
	...
	private static function create_dir($path, $name = '')
	{
		if (file_exists($path))
		{
			if (is_dir($path))
			{
				throw new Kohana_Exception('Could not write to :name directory', array('name' => $name));
			}
			else
			{
				throw new Kohana_Exception(':name path is a file', array('name' => $name));
			}
		}
		else
		{
			...
			{
				throw new Kohana_Exception('Could not create :name directory', array('name' => $name));
			}
			if (!is_writeable($path))
			{
				throw new Kohana_Exception('Created :name directory but could not write to it', array('name' => $name));
			}
		}
	}
...
//			$smarty->addTemplateDir($dirs);
	...
	public function set_filename($file)
	{
		...
		{
			throw new Kohana_Exception('Cannot use set_filename to change from a Smarty template to :tpl.', array(':tpl' => $file));
		}
		
		if(strpos($file, DOCROOT) !== FALSE AND file_exists($file))
		{
			$path = $file;
		}
